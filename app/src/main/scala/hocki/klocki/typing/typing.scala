...
def instantiateSchemata(toplevel: Toplevel, nr: ResolvedNames): Map[SchemaBinding, Schema] =
  ...
  val definedIfaces = constructDefinedIfaces(nr.schemaDefs, schemaDims, schemaDsvs)
  
  instantiateSchemata(nr.schemaDefs, schemaDims, schemaDsvs, definedIfaces)
...
(using nr: ResolvedNames): Map[SchemaBinding, Set[Renamer]] =
  impl.blockUses.map(use =>
    ...
    val usedIface = constructUsedIface(dims, usedSchema.dimArgs, use.iface)
    binding -> Renamer(definedIfaces(binding), usedIface)
  ).groupMap(_._1)(_._2)
